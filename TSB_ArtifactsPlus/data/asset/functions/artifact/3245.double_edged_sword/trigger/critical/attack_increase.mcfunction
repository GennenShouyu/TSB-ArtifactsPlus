...
# 効果音
    ...
    execute at @e[type=#lib:living,tag=1B9.Target,limit=1] run particle minecraft:firework ~ ~1 ~ 0.4 1.0 0.4 0.5 20

# ダメージ
    data modify storage api: Argument.Damage set value 9999
    data modify storage api: Argument.AttackType set value "Physical"
    function api:damage/modifier
    execute as @e[type=#lib:living,tag=1B9.Target,limit=1] run function api:damage/
