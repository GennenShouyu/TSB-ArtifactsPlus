#> asset:artifact/3236.tank_gun/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/3236.tank_gun/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 3236
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:carrot_on_a_stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"戦車砲","color":"#bb0000","bold":true,"underlined":true}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"戦車からもぎ取った大砲。"}','{"text":"発射すれば、周囲の敵と地形を全て吹き飛ばす。"}','{"text":"但し、とても生身の人間が扱えるものでは無い。"}','{"text":" "}','{"text":"島丸ごと吹っ飛ばしちまえ！！！","color":"gray"}']
...
# 攻撃に関する情報 -Damage量 (literal[]/literal) Wikiを参照 (オプション)
    data modify storage asset:artifact AttackInfo.Damage set value [3200]
...
# 神器のクールダウン (int) (オプション)
    data modify storage asset:artifact LocalCooldown set value 3000
