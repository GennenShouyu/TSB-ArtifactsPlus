#> asset:artifact/3261.the_elder_wand/trigger/magic/hit
#
#
#
# @within function asset:artifact/3261.the_elder_wand/trigger/magic/bullet

# 一定の距離から距離減衰するための距離(適正距離をメートル*2で指定)
    scoreboard players set $Appropriate_Distance Temporary 16
# 最低ダメージ保証
    scoreboard players set $MinDamage Temporary 280
...
# ダメージ設定
    ...
    # もし、適正距離じゃなかった場合、距離-適正距離=減少ダメージにする
        execute if score $Distance_Damping Temporary >= $Appropriate_Distance Temporary run function asset:artifact/3261.the_elder_wand/trigger/magic/damage_attenuation
    # ダメージ量確定
