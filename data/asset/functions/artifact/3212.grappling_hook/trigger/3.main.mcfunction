#> asset:artifact/3212.grappling_hook/trigger/3.main
#
# 神器のメイン処理部
#
# @within function asset:artifact/3212.grappling_hook/trigger/2.check_condition

# 基本的な使用時の処理(MP消費や使用回数の処理など)を行う mainhand/feet/legs/chest/head/mainhand/offhandを記載してね
    function asset:artifact/common/use/mainhand

# ここから先は神器側の効果の処理を書く

# 弾を進めるための再起開始
    execute positioned ~ ~1.5 ~ run function asset:artifact/3212.grappling_hook/trigger/3.1.bullet
# タグを消す
    tag @s remove Landing

# 演出
    playsound minecraft:entity.generic.explode player @a ~ ~ ~ 1 1.5
    playsound minecraft:entity.generic.explode player @a ~ ~ ~ 1 2
    playsound minecraft:entity.blaze.shoot player @a ~ ~ ~ 1 0.75
    playsound minecraft:entity.wither.hurt player @a ~ ~ ~ 1 1.5

# 残り回数が1回の時発動した場合
    execute unless data storage asset:context Items.mainhand.id run data modify storage api: Argument.ID set value 3213
    execute unless data storage asset:context Items.mainhand.id run function api:artifact/replace/from_id

# HP回復
    data modify storage api: Argument.Heal set value 20
    function api:heal/modifier
    function api:heal/
