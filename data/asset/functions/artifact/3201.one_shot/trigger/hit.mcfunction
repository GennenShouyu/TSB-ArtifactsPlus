#> asset:artifact/3201.one_shot/trigger/hit
#
#
# @within function
#    asset:artifact/3201.one_shot/trigger/3.main
#    asset:artifact/3201.one_shot/trigger/bullet
...
# 最低ダメージ保証
    scoreboard players set $MinDamage Temporary 3500

# 演出
    execute at @e[tag=LandingTarget,limit=1] run particle minecraft:block redstone_block ~ ~1.2 ~ 0.4 0.4 0.4 0 4
# ダメージ設定
    # ダメージ
        scoreboard players add $Damage Temporary 4900
    # 属性
        data modify storage api: Argument.AttackType set value "Physical"
        data modify storage api: Argument.ElementType set value "None"
    # もし、適正距離じゃなかった場合、距離-適正距離=減少ダメージにする
        execute if score $Distance_Damping Temporary >= $Appropriate_Distance Temporary run function asset:artifact/3201.one_shot/trigger/damage_attenuation
    # ダメージ量確定
