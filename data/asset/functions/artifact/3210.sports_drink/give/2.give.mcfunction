#> asset:artifact/3210.sports_drink/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/3210.sports_drink/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 3210
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:honey_bottle"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '[{"text":"スポーツドリンク","color":"aqua"}]'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['[{"text":"1分間、移動速度を10%増加させ、MPを少しずつ回復させる。"}]','[{"text":"　"}]','[{"text":"「どこかで見たことあるようなイオン飲料だ。」","color":"gray"}]']
