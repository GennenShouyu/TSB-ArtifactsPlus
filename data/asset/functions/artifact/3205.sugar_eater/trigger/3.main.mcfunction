...
# 神器のメイン処理部
#
# @within function asset:artifact/3205.sugar_eater/trigger/2.check_condition
...
# 演出
    ...
    playsound entity.blaze.hurt player @a ~ ~ ~ 1 0.5
    playsound item.axe.wax_off player @a ~ ~ ~ 2 1
    playsound block.sand.break player @a ~ ~ ~ 5 1

# 残り回数が1回の時発動した場合
    execute unless data storage asset:context Items.mainhand.id run data modify storage api: Argument.ID set value 3206
...
# 発砲
    execute anchored eyes positioned ^-0.35 ^-0.15 ^ facing entity @e[type=marker,tag=SpreadMarker,limit=1] feet run function asset:artifact/3205.sugar_eater/trigger/bullet
