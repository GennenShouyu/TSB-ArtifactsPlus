#> asset:artifact/3226.sunk_heart/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/3226.sunk_heart/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 3226
# 神器のベースアイテム
    data modify storage asset:artifact Item set value "minecraft:stick"
# 神器の名前 (TextComponentString)
    data modify storage asset:artifact Name set value '{"text":"沈んだ心","color":"#7590c2"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"泡を立てて沈むハートを模したペンダント。","color":"white"}','{"text":"移動速度を20%失うが、水攻撃+15%を得る。","color":"white"}','{"text":" ","color":"white"}','{"text":"どうしようもなく、自分が嫌いだ。","color":"gray","italic":false}']
...
# 装備時ステータス補正 ({Type: string, Amount: double, Operation: string, StackReduction?: double}[]) (オプション)
    ...
    data modify storage asset:artifact Equipment.Modifiers append value {Type:"attack/water",Amount:0.15d,Operation:"multiply_base"}
